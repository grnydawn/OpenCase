...
class BeginSource(BeginStatement):
    ...
    """
    f2003_class = Fortran2003.Begin_Source # OC addition
...
class EndModule(EndStatement):
    f2003_class = Fortran2003.End_Module_Stmt # OC addition
...
class Module(BeginStatement, HasAttributes,
    ...
    END [MODULE [name]]
    """
    f2003_class = Fortran2003.Module_Stmt # OC addition
...
class EndProgram(EndStatement):
    ...
    END [PROGRAM [name]]
    """
    f2003_class = Fortran2003.End_Program_Stmt # OC addition

    match = re.compile(r'end(\s*program\s*\w*|)\Z', re.I).match

class Program(BeginStatement, ProgramBlock,
              HasAttributes, # XXX: why Program needs .attributes? # uncommended OC
              HasVariables,
              HasImplicitStmt, HasUseStmt, AccessSpecs):
    """ PROGRAM [name]
    """
    f2003_class = Fortran2003.Program_Stmt # OC addition
...
class EndInterface(EndStatement):
    f2003_class = Fortran2003.End_Interface_Stmt # OC addition
...
class Interface(BeginStatement, HasAttributes, HasImplicitStmt, HasUseStmt,
    ...
    """
    f2003_class = Fortran2003.Interface_Stmt # OC addition
...
class SubProgramStatement(BeginStatement, ProgramBlock,
    ...
    def tooc(self, **kwargs):
        if len(kwargs)>0:
            ...
            construct_name = self.construct_name
            construct_name = construct_name + ': ' if construct_name else ''
            return self.get_indent_tab(isfix=None).lstrip() + construct_name + self.tostr()
            #return super(SubProgramStatement, self).tooc()
    # end of OC
    ...
    def tostr(self, to_subr=False):
        ...
        # end of OC addition
        #clsname = self.__class__.__name__.upper() # OC deletion
        s = ''
        if self.prefix:
            s += self.prefix + ' '
        #if self.typedecl is not None: # OC deletion
        if not to_subr and self.typedecl is not None: # OC addition
            assert isinstance(self, Function),`self.__class__.__name__`
            if not hasattr(self, 'result_in_typedecl') or not self.result_in_typedecl: # OC addition
                s += self.typedecl.tostr() + ' ' # OC addition
            #s += self.typedecl.tostr() + ' ' # OC deletion
        s += clsname
        suf = ''
        #if self.result and self.result!=self.name: # OC deletion
        if not to_subr and self.result and self.result!=self.name: # OC addition
...
class EndSubroutine(EndStatement):
    ...
    END [SUBROUTINE [name]]
    """
    f2003_class = Fortran2003.End_Subroutine_Stmt # OC addition
...
class Subroutine(SubProgramStatement):
    ...
    """
    f2003_class = Fortran2003.Subroutine_Stmt # OC addition
...
class EndFunction(EndStatement):
    ...
    END [FUNCTION [name]]
    """
    f2003_class = Fortran2003.End_Function_Stmt # OC addition
...
class Function(SubProgramStatement):
    ...
    """
    f2003_class = Fortran2003.Function_Stmt # OC addition
...
class EndSelect(EndStatement):
    f2003_class = Fortran2003.End_Select_Stmt # OC addition
...
class Select(BeginStatement):
    ...
    """
    f2003_class = Fortran2003.Select_Case_Stmt # OC addition
...
class EndWhere(EndStatement):
    ...
    """
    f2003_class = Fortran2003.End_Where_Stmt # OC addition
...
class Where(BeginStatement):
    ...
    """
    f2003_class = Fortran2003.Where_Construct_Stmt # OC addition
...
class EndIfThen(EndStatement):
    ...
    """
    f2003_class = Fortran2003.End_If_Stmt # OC addition
...
class IfThen(BeginStatement):
    ...
    IfThen instance has the following attributes:
      expr
    """
    f2003_class = Fortran2003.If_Then_Stmt # OC addition
...
class If(BeginStatement):
    ...
    """
    f2003_class = Fortran2003.If_Stmt # OC addition
...
class EndDo(EndStatement):
    ...
    """
    f2003_class = Fortran2003.End_Do_Stmt # OC addition
...
class Do(BeginStatement):
    ...
    """
    f2003_class = Fortran2003.Do_Stmt # OC addition
    ...
    # start of OC addition
    def tostr(self, label=None, var=None, start=None, end=None, step=None):
        l = ['DO']

        if label:
            l.append(label)
        elif self.endlabel:
            l.append(self.endlabel)

        if self.loopcontrol:
            loopcontrol = self.loopcontrol.split(',')
        else: raise Exception('No loop control in Do statement')

        loopvar, loopstart = loopcontrol[0].split('=')
        if var:
            if start:
                l.append('%s=%s'%(var,start))
            else:
                l.append('%s=%s'%(var,loopstart))
        else:
            if start:
                l.append('%s=%s'%(loopvar,start))
            else:
                l.append('%s=%s'%(loopvar,loopstart))

        if end:
            l.append(end)
        else:
            l.append(loopcontrol[1])

        if step:
            l.append(step)
        else:
            if len(loopcontrol)==3:
                l.append(loopcontrol[2])
            else:
                l.append('1')
...
class EndType(EndStatement):
    ...
    END TYPE [<type-name>]
    """
    f2003_class = Fortran2003.End_Type_Stmt # OC addition
...
class Type(BeginStatement, HasVariables, HasAttributes, AccessSpecs):
    ...
    <type-attr-spec> = <access-spec> | EXTENDS ( <parent-type-name> )
                       | ABSTRACT | BIND(C)
    """
    f2003_class = Fortran2003.Derived_Type_Stmt # OC addition
...
#R214
#action_stmt = [ Allocate, GeneralAssignment, Assign, Backspace, Call, Close, # OC deletion
action_stmt = [ Allocate, GeneralAssignment, Assignment, Assign, Backspace, Call, Close, # OC addition
...
#executable_construct = [ Associate, Do, ForallConstruct, IfThen, # OC deletion
executable_construct = [ Associate, Do, ForallStmt, ForallConstruct, IfThen, # OC addition
