...
class TypeDeclarationStatement(Statement):
    ...
    """
    f2003_class = Fortran2003.Type_Declaration_Stmt # OC addition
    ...
    def tooc(self, **kwargs):
        if len(kwargs)>0:
            ...
            return super(TypeDeclarationStatement, self).tooc()
    ...
    def tofortran(self,isfix=None):
        ...
        if self.attrspec:
            ...
            #if self.entity_decls: # OC deletion
            #    s += ' ::' # OC deletion
        if self.entity_decls:
            #s += ' ' + ', '.join(self.entity_decls) # OC deletion
            s += ' :: ' + ', '.join(self.entity_decls) # OC addition
        return tab + s
...
class Implicit(Statement):
    ...
    """
    f2003_class = Fortran2003.Implicit_Stmt # OC addition
