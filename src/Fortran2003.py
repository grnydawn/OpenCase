...
logger = logging.getLogger("opencase")


# start of OC

class Comment(object):
    def tofortran(self, **kwargs): # OC addition
        return '!comment line'
...
##         return
    ...
    #def tofortran(self, tab='', isfix=None): # OC deletion
    #    return tab + str(self) # OC deletion

    def tofortran(self, tab='', isfix=None, **kwargs): # OC addition
        return tab + self.tostr(**kwargs) # OC addition
    ...
    @classmethod
    def _tostr(cls, obj, **kwargs):
        if hasattr(obj, 'tostr'):
            return obj.tostr(**kwargs)
            ...
            for o in obj:
                if hasattr(o, 'tostr'):
                    l.append(o.tostr(**kwargs))
                else:
                    l.append(Base._tostr(o))
            return obj.__class__(l)
        else: raise Exception('Unknown type: %s'%obj.__class__)
    # end of OC addition
...
content : tuple
    ...
    #def tostr(self): # OC deletion
    #    return self.tofortran() # OC deletion

    def tostr(self, **kwargs): # OC addition
        return self.tofortran(**kwargs) # OC addition
    ...
    def tofortran(self, tab='', isfix=None, **kwargs): # OC addition
        ...
        if start is not None:
            #l.append(start.tofortran(tab=tab,isfix=isfix)) # OC deletion
            l.append(start.tofortran(tab=tab,isfix=isfix, **kwargs)) # OC addition
        for item in self.content[1:-1]:
            #l.append(item.tofortran(tab=tab+extra_tab,isfix=isfix)) # OC deletion
            l.append(item.tofortran(tab=tab+extra_tab,isfix=isfix, **kwargs)) # OC addition
        if len(self.content)>1:
            #l.append(end.tofortran(tab=tab,isfix=isfix)) # OC deletion
            l.append(end.tofortran(tab=tab,isfix=isfix, **kwargs)) # OC addition
        return '\n'.join(l)
...
::
    ...
    def tostr(self, **kwargs):
        ...
        #return s.join(map(str, self.items)) # OC deletion
        return s.join(Base._tostr(self.items, **kwargs)) # OC addition
...
::
    ...
    """
    def tostr(self, **kwargs):
        #return '%s %s' % tuple(self.items) # OC deletion
        return '%s %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
::
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #return '%s %s %s' % tuple(self.items) # OC deletion
        return '%s %s %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
::
    ...
    def tostr(self, **kwargs):
        ...
        if self.items[0] is not None:
            #s += '%s :' % (self.items[0]) # OC deletion
            s += '%s :' % Base._tostr((self.items[0]), **kwargs) # OC addition
        else:
            s += ':'
        if self.items[1] is not None:
            #s += ' %s' % (self.items[1]) # OC addition
            s += ' %s' % Base._tostr((self.items[1]), **kwargs) # OC deletion
        return s
...
::
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if self.items[0] is None: return str(self.items[1]) # OC deletion
        #return '%s = %s' % tuple(self.items) # OC deletion
        if self.items[0] is None: return Base._tostr(self.items[1], **kwargs) # OC addition
        return '%s = %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
::
    ...
    def tostr(self, **kwargs):
        if self.items[1] is None:
            #return '%s%s' % (self.items[0], self.items[2]) # OC deletion
            return '%s%s' % Base._tostr((self.items[0], self.items[2]), **kwargs) # OC addition
        #return '%s%s%s' % tuple(self.items) # OC deletion
        return '%s%s%s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
::
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if self.items[1] is None: return str(self.items[0]) # OC deletion
        #return '%s_%s' % tuple(self.items) # OC deletion
        if self.items[1] is None: return Base._tostr(self.items[0], **kwargs) # OC addition
        return '%s_%s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
    def compare(self, other):
...
::
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if self.items[1] is None: return '%s()' % (self.items[0]) # OC deletion
        #return '%s(%s)' % (self.items[0], self.items[1]) # OC deletion
        if self.items[1] is None: return '%s()' % Base._tostr((self.items[0]), **kwargs) # OC addition
        return '%s(%s)' % Base._tostr((self.items[0], self.items[1]), **kwargs) # OC addition
...
string
    ...
    def init(self, string):
        self.string = string
        return
    #def tostr(self, **kwargs): return str(self.string) # OC deletion
    def tostr(self, **kwargs): return Base._tostr(self.string, **kwargs) # OC addition
...
item : readfortran.Line
    ...
    def tofortran(self, tab='', isfix=None, **kwargs): # OC addition
        ...
        if isfix:
            ...
            c = ''
        #if label: # OC deletion
        if label and (not kwargs.has_key('remove_label') or not kwargs['remove_label']): # OC addition
            #t = c + str(label) # OC deletion
            t = c + Base._tostr(label, **kwargs) # OC addition
            if isfix:
                while len(t)<6: t += ' '
            else:
                tab = tab[len(t):] or ' '
            ...
            t = ''
        #if name: # OC deletion
        if name and (not kwargs.has_key('remove_name') or not kwargs['remove_name']): # OC addition
            #return t + tab + name+':' + str(self) # OC deletion
            return t + tab + name+':' + Base._tostr(self) # OC addition
        #return t + tab + str(self) # OC deletion
        return t + tab + Base._tostr(self, **kwargs) # OC addition
...
::
    ...
    def tostr(self, **kwargs):
        if self.items[1] is not None:
            #return 'END %s %s' % tuple(self.items) # OC deletion
            return 'END %s %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
        #return 'END %s' % (self.items[0]) # OC deletion
        return 'END %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
    def torepr(self):
...
::
    ...
    def tostr(self, **kwargs):
        ...
        # start of OC addition
        if self.items[1] is None: return Base._tostr(self.items[0], **kwargs)
        s = Base._tostr(self.items[1], **kwargs)
        if s and s[0] in '(*':
            return '%s%s' % Base._tostr((self.items[0], s), **kwargs)
        return '%s %s' % Base._tostr((self.items[0], s), **kwargs)
        # end of OC addition

    def tostr_a(self, **kwargs): # colons version of tostr
        #if self.items[1] is None: return str(self.items[0]) # OC deletion
        #return '%s :: %s' % (self.items[0], self.items[1]) # OC deletion
        if self.items[1] is None: return Base._tostr(self.items[0], **kwargs) # OC addition
        return '%s :: %s' % Base._tostr((self.items[0], self.items[1]), **kwargs) # OC addition
...
class Type_Declaration_StmtBase(StmtBase):
    ...
    def tostr(self, **kwargs):
        if self.items[1] is None:
            ...
            #return '%s, %s :: %s' % self.items # OC deletion
            return '%s, %s :: %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Name(StringBase): # R304
    ...
    def tostr(self, **kwargs):
        ...
        for oldname, newname in kwargs.get('name_rename', []):
            m = re.match(r'^%s$'%oldname, self.string)
            if m: return newname

        for lname, rname in kwargs.get('name_switch', []):
            m = re.match(lname, self.string)
            if m: return rname
            m = re.match(rname, self.string)
            if m: return lname

        return super(Name, self).tostr(**kwargs)        
    # end of OC addition
...
class Kind_Selector(Base): # R404
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if len(self.items)==2: return '%s%s' % tuple(self.items) # OC deletion
        #return '%sKIND = %s%s' % tuple(self.items) # OC deletion
        if len(self.items)==2: return '%s%s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
        return '%sKIND = %s%s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
class Complex_Literal_Constant(Base): # R421
    ...
    match = staticmethod(match)
    #def tostr(self, **kwargs): return '(%s, %s)' % tuple(self.items) # OC deletion
    def tostr(self, **kwargs): return '(%s, %s)' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
class Char_Selector(Base): # R424
    ...
    def tostr(self, **kwargs):
        if self.items[0] is None:
            #return '(KIND = %s)' % (self.items[1]) # OC deletion
            return '(KIND = %s)' % Base._tostr((self.items[1]), **kwargs) # OC addition
        #return '(LEN = %s, KIND = %s)' % (self.items[0],self.items[1]) # OC deletion
        return '(LEN = %s, KIND = %s)' % Base._tostr((self.items[0],self.items[1]), **kwargs) # OC addition
...
class Length_Selector(Base): # R425
    ...
    def tostr(self, **kwargs):
        #if len(self.items)==2: return '%s%s' % tuple(self.items) # OC deletion
        #return '%sLEN = %s%s' % tuple(self.items) # OC deletion
        if len(self.items)==2: return '%s%s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
        return '%sLEN = %s%s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
class Char_Literal_Constant(Base): # R427
    ...
    def tostr(self, **kwargs):
        #if self.items[1] is None: return str(self.items[0]) # OC deletion
        #return '%s_%s' % (self.items[1], self.items[0]) # OC deletion
        if self.items[1] is None: return Base._tostr(self.items[0], **kwargs) # OC addition
        return '%s_%s' % Base._tostr((self.items[1], self.items[0]), **kwargs) # OC addition
...
class Derived_Type_Stmt(StmtBase): # R430
    ...
    def tostr(self, **kwargs):
        s = 'TYPE'
        if self.items[0] is not None:
            #s += ', %s :: %s' % (self.items[0], self.items[1]) # OC deletion
            s += ', %s :: %s' % Base._tostr((self.items[0], self.items[1]), **kwargs) # OC addition
        else:
            #s += ' :: %s' % (self.items[1]) # OC deletion
            s += ' :: %s' % Base._tostr((self.items[1]), **kwargs) # OC addition
        if self.items[2] is not None:
            #s += '(%s)' % (self.items[2]) # OC deletion
            s += '(%s)' % Base._tostr((self.items[2]), **kwargs) # OC addition
        return s
...
class Type_Attr_Spec(Base): # R431
    ...
    def tostr(self, **kwargs):
        if self.items[1] is None:
            #return '%s' % (self.items[0]) # OC deletion
            return '%s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return '%s(%s)' % (self.items) # OC deletion
        return '%s(%s)' % Base._tostr((self.items), **kwargs) # OC addition
...
class Type_Param_Def_Stmt(StmtBase): # R435
    ...
    def tostr(self, **kwargs):
        s = 'INTEGER'
        if self.items[0] is not None:
            #s += '%s, %s :: %s' % tuple(self.items) # OC deletion
            s += '%s, %s :: %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
        else:
            #s += ', %s :: %s' % tuple(self.items[1:]) # OC deletion
            s += ', %s :: %s' % tuple(Base._tostr(self.items[1:], **kwargs)) # OC addition
        return s
...
class Component_Part(BlockBase): # R438
    ...
    def tofortran(self, tab='', isfix=None, **kwargs): # OC addition
        ...
        for item in self.content:
            #l.append(item.tofortran(tab=tab,isfix=isfix)) # OC deletion
            l.append(item.tofortran(tab=tab,isfix=isfix, **kwargs)) # OC addition
        return '\n'.join(l)
...
class Component_Decl(Base): # R442
    ...
    def tostr(self, **kwargs):
        s = str(self.items[0])
        if self.items[1] is not None:
            #s += '(' + str(self.items[1]) + ')' # OC deletion
            s += '(' + Base._tostr(self.items[1], **kwargs) + ')' # OC addition
        if self.items[2] is not None:
            #s += '*' + str(self.items[2]) # OC deletion
            s += '*' + Base._tostr(self.items[2], **kwargs) # OC addition
        if self.items[3] is not None:
            #s += ' ' + str(self.items[3]) # OC deletion
            s += ' ' + Base._tostr(self.items[3], **kwargs) # OC addition 
        return s
...
class Proc_Component_Def_Stmt(StmtBase): # R445
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            #return 'PROCEDURE(%s), %s :: %s' % (self.items) # OC deletion
            return 'PROCEDURE(%s), %s :: %s' % Base._tostr((self.items), **kwargs) # OC addition
        #return 'PROCEDURE(), %s :: %s' % (self.items[1:]) # OC deletion
        return 'PROCEDURE(), %s :: %s' % Base._tostr((self.items[1:]), **kwargs) # OC addition
...
class Specific_Binding(StmtBase): # R451
    ...
    def tostr(self, **kwargs):
        r = 'PROCEDURE'
        if self.items[0] is not None:
            #r += '(%s)' % (self.items[0]) # OC deletion
            r += '(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
        if self.items[1] is not None:
            #r += ', %s ::' % (self.items[1]) # OC deletion
            r += ', %s ::' % Base._tostr((self.items[1]), **kwargs) # OC addition
        #r += ' %s' % (self.items[2]) # OC deletion
        r += ' %s' % Base._tostr((self.items[2]), **kwargs) # OC addition
        if self.items[3] is not None:
            #r += ' => %s' % (self.items[3]) # OC deletion
            r += ' => %s' % Base._tostr((self.items[3]), **kwargs) # OC addition
        return r        
...
class Generic_Binding(StmtBase): # R452
    ...
    def tostr(self, **kwargs):
        if self.items[0] is None:
            #return 'GENERIC :: %s => %s' % (self.items[1:]) # OC deletion
            return 'GENERIC :: %s => %s' % Base._tostr((self.items[1:]), **kwargs) # OC addition
        #return 'GENERIC, %s :: %s => %s' % (self.items) # OC deletion
        return 'GENERIC, %s :: %s => %s' % Base._tostr((self.items), **kwargs) # OC addition
...
class Ac_Spec(Base): # R466
    ...
    def tostr(self, **kwargs):
        if self.items[0] is None:
            #return str(self.items[1]) # OC deletion
            return Base._tostr(self.items[1], **kwargs) # OC addition
        if self.items[1] is None:
            #return str(self.items[0]) + ' ::' # OC deletion
            return Base._tostr(self.items[0], **kwargs) + ' ::' # OC addition
        #return '%s :: %s' % self.items # OC deletion
        return '%s :: %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Ac_Implied_Do_Control(Base): # R471
    ...
    match = staticmethod(match)
    #def tostr(self, **kwargs): return '%s = %s' % (self.items[0], ', '.join(map(str,self.items[1]))) # OC deletion
    def tostr(self, **kwargs): # OC addition
        return '%s = %s' % (Base._tostr(self.items[0], **kwargs), ', '.join(Base._tostr(self.items[1], **kwargs))) # OC addition
...
class Declaration_Type_Spec(Base): # R502
    ...
    def match(string):
        ...
        return
    match = staticmethod(match)
    #def tostr(self, **kwargs): return '%s(%s)' % self.items # OC deletion
    def tostr(self, **kwargs): return '%s(%s)' % Base._tostr(self.items, **kwargs) # OC addition
...
:F03R:`509`::
    ...
    def tostr(self, **kwargs):
        ...
        #return 'BIND(C, NAME = %s)' % (self.items[0]) # OC deletion
        return 'BIND(C, NAME = %s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
class Explicit_Shape_Spec(SeparatorBase): # R511
    ...
    def tostr(self, **kwargs):
        #if self.items[0] is None: return str(self.items[1]) # OC deletion
        #return SeparatorBase.tostr(self, **kwargs) # OC deletion
        if self.items[0] is None: return Base._tostr(self.items[1], **kwargs) # OC addition
        return SeparatorBase.tostr(self, **kwargs) # OC addition
...
:F03R:`516`::
    ...
    def tostr(self, **kwargs):
        s = ''
        if self.items[0] is not None:
            #s += str(self.items[0]) + ', ' # OC deletion
            s += Base._tostr(self.items[0], **kwargs) + ', ' # OC addition
        if self.items[1] is not None:
            #s += str(self.items[1]) + ' : ' # OC deletion
            s += Base._tostr(self.items[1], **kwargs) + ' : ' # OC addition
        s += '*'
...
:F03R:524::
    ...
    def tostr(self, **kwargs):
        #return 'DATA ' + ', '.join(map(str, self.items)) # OC deletion
        return 'DATA ' + ', '.join(Base._tostr(self.items, **kwargs)) # OC addition
...
:F03R:527::
    ...
    def tostr(self, **kwargs):
        #l = '%s, %s = %s, %s' % tuple(self.items[:4]) # OC deletion
        l = '%s, %s = %s, %s' % tuple(Base._tostr(self.items[:4], **kwargs)) # OC addition
        if self.items[4] is not None:
            #l += ', %s' % (self.items[4]) # OC deletion
            l += ', %s' % Base._tostr((self.items[4]), **kwargs) # OC addition
        return '('+l+')'
...
class Dimension_Stmt(StmtBase): # R535
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #return 'DIMENSION :: ' + ', '.join(['%s(%s)' % ns for ns in self.items[0]]) # OC deletion
        return 'DIMENSION :: ' + ', '.join(['%s(%s)' % Base._tostr(ns, **kwargs) for ns in self.items[0]]) # OC addition
...
class Intent_Stmt(StmtBase): # R536
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #return 'INTENT(%s) :: %s' % self.items # OC deletion
        return 'INTENT(%s) :: %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Target_Stmt(StmtBase): # R546
    ...
    def tostr(self, **kwargs):
        #return 'TARGET :: %s' % (self.items[0]) # OC deletion
        return 'TARGET :: %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
items : ({'NONE', Implicit_Spec_List},)
    ...
    def tostr(self, **kwargs):
        #return 'IMPLICIT %s' % (self.items[0]) # OC deletion
        return 'IMPLICIT %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
items : (Namelist_Group_Name, Namelist_Group_Object_List)-tuple
    ...
    def tostr(self, **kwargs):
        #return 'NAMELIST ' + ', '.join('/%s/ %s' % (name_lst) for name_lst in self.items) # OC deletion
        return 'NAMELIST ' + ', '.join('/%s/ %s' % Base._tostr((name_lst), **kwargs) for name_lst in self.items) # OC addition
...
class Common_Stmt(StmtBase): # R557
    ...
    def tostr(self, **kwargs):
        ...
        for (name, lst) in self.items[0]:
            if name is not None:
                #s += ' /%s/ %s' % (name, lst) # OC deletion
                s += ' /%s/ %s' % Base._tostr((name, lst), **kwargs) # OC addition
            else:
                #s += ' // %s' % (lst) # OC deletion
                s += ' // %s' % Base._tostr((lst), **kwargs) # OC addition
        return s
...
class Allocate_Stmt(StmtBase): # R623
    ...
    def tostr(self, **kwargs):
        ...
        if spec is not None:
            if opts is not None:
                #return 'ALLOCATE(%s::%s, %s)' % (spec, lst, opts) # OC deletion
                return 'ALLOCATE(%s::%s, %s)' % Base._tostr((spec, lst, opts), **kwargs) # OC addition
            else:
                #return 'ALLOCATE(%s::%s)' % (spec, lst) # OC deletion
                return 'ALLOCATE(%s::%s)' % Base._tostr((spec, lst), **kwargs) # OC addition
        elif opts is not None:
            #return 'ALLOCATE(%s, %s)' % (lst, opts) # OC deletion
            return 'ALLOCATE(%s, %s)' % Base._tostr((lst, opts), **kwargs) # OC addition
        else:
            #return 'ALLOCATE(%s)' % (lst) # OC deletion
            return 'ALLOCATE(%s)' % Base._tostr((lst), **kwargs) # OC addition
...
class Allocate_Shape_Spec(SeparatorBase): # R630
    ...
    def tostr(self, **kwargs):
        #if self.items[0] is None: return str(self.items[1]) # OC deletion
        #return SeparatorBase.tostr(self) # OC deletion
        if self.items[0] is None: return Base._tostr(self.items[1], **kwargs) # OC addition
        return SeparatorBase.tostr(self, **kwargs) # OC addition
...
class Deallocate_Stmt(StmtBase): # R635
    ...
    def tostr(self, **kwargs):
        if self.items[1] is not None:
            #return 'DEALLOCATE(%s, %s)' % (self.items) # OC deletion
            return 'DEALLOCATE(%s, %s)' % Base._tostr((self.items), **kwargs) # OC addition
        #return 'DEALLOCATE(%s)' % (self.items[0]) # OC deletion
        return 'DEALLOCATE(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
class Where_Stmt(StmtBase): # R743
    ...
    match = staticmethod(match)
    #def tostr(self, **kwargs): return 'WHERE (%s) %s' % tuple(self.items) # OC deletion
    def tostr(self, **kwargs): return 'WHERE (%s) %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
class Where_Construct_Stmt(StmtBase): # R745
    ...
    #def tostr(self, **kwargs): return 'WHERE (%s)' % tuple(self.items) # OC deletion
    def tostr(self, **kwargs): return 'WHERE (%s)' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
class Masked_Elsewhere_Stmt(StmtBase): # R749
    ...
    def tostr(self, **kwargs):
        #if self.items[1] is None: return 'ELSEWHERE(%s)' % (self.items[0]) # OC deletion
        #return 'ELSEWHERE(%s) %s' % self.items # OC deletion
        if self.items[1] is None: return 'ELSEWHERE(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
        return 'ELSEWHERE(%s) %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Forall_Triplet_Spec(Base): # R755
    ...
    def tostr(self, **kwargs):
        if self.items[3] is None:
            #return '%s = %s : %s' % (self.items[:3]) # OC deletion
            return '%s = %s : %s' % Base._tostr((self.items[:3]), **kwargs) # OC addition
        #return '%s = %s : %s : %s' % (self.items) # OC deletion
        return '%s = %s : %s : %s' % Base._tostr((self.items), **kwargs) # OC addition
...
class Forall_Stmt(StmtBase): # R759
    ...
    def match(string):
        ...
        #if not line.startswith(')'): return # OC deletion
        if not line.startswith('('): return # OC addition
        i = line.find(')')
        if i==-1: return
        ...
        line = repmap(line[i+1:].lstrip())
        if not line: return
        #return Forall_Header(header), Forall_Assignment_Stmt(line) # OC deletion
        return Forall_Header('('+header+')'), Forall_Assignment_Stmt(line) # OC addition

    #def tostr(self, **kwargs): return 'FORALL %s %s' % self.items # OC deletion
    def tostr(self, **kwargs): return 'FORALL %s %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class If_Then_Stmt(StmtBase): # R803
    ...
    def tostr(self, **kwargs):
        #return 'IF (%s) THEN' % self.items # OC deletion
        return 'IF (%s) THEN' % Base._tostr(self.items, **kwargs) # OC addition
...
class Else_If_Stmt(StmtBase): # R804
    ...
    def tostr(self, **kwargs):
        if self.items[1] is None:
            #return 'ELSE IF (%s) THEN' % (self.items[0]) # OC deletion
            return 'ELSE IF (%s) THEN' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'ELSE IF (%s) THEN %s' % self.items # OC deletion
        return 'ELSE IF (%s) THEN %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Else_Stmt(StmtBase): # R805
    ...
    def tostr(self, **kwargs):
        if self.items[0] is None:
            return 'ELSE'
        #return 'ELSE %s' % self.items # OC deletion
        return 'ELSE %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class If_Stmt(StmtBase): # R807
    ...
    #def tostr(self, **kwargs): return 'IF (%s) %s' % self.items # OC deletion
    def tostr(self, **kwargs): return 'IF (%s) %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Select_Case_Stmt(StmtBase, CALLBase): # R809
    ...
    def tostr(self, **kwargs):
        #return 'SELECT CASE (%s)' % (self.items[0]) # OC deletion
        return 'SELECT CASE (%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
class Case_Stmt(StmtBase): # R810
    ...
    def tostr(self, **kwargs):
        if self.items[1] is None:
            #return 'CASE %s' % (self.items[0]) # OC deletion
            return 'CASE %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'CASE %s %s' % (self.items) # OC deletion
        return 'CASE %s %s' % Base._tostr((self.items), **kwargs) # OC addition
...
class Select_Type_Stmt(StmtBase): # R822
    ...
    def tostr(self, **kwargs):
        if self.items[0] is None:
            #return 'SELECT TYPE(%s)' % (self.items[1]) # OC deletion
            return 'SELECT TYPE(%s)' % Base._tostr((self.items[1]), **kwargs) # OC addition
        #return 'SELECT TYPE(%s=>%s)' % (self.items) # OC deletion
        return 'SELECT TYPE(%s=>%s)' % Base._tostr((self.items), **kwargs) # OC addition
...
class Label_Do_Stmt(StmtBase): # R828
    ...
    def tostr(self, **kwargs):
        ...
        if name is None:
            #s = 'DO %s' % (label) # OC deletion
            s = 'DO %s' % Base._tostr((label), **kwargs) # OC addition
        else:
            #s = '%s: DO %s' % (label) # OC deletion
            s = '%s: DO %s' % Base._tostr((label), **kwargs) # OC addition
        if loop_control is not None:
            #s += ' %s' % (loop_control) # OC deletion
            s += ' %s' % Base._tostr((loop_control), **kwargs) # OC addition
        return s
...
class Loop_Control(Base): # R830
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if len(self.items)==1: return ', WHILE (%s)' % (self.items[0]) # OC deletion
        #return ', %s = %s' % (self.items[0], ', '.join(map(str,self.items[1]))) # OC deletion

        # start of OC addition
        items = list(self.items)
        items[1] = list(self.items[1])

        if kwargs.get('do_var', None): items[0] = kwargs['do_var']
        if kwargs.get('do_start', None): items[1][0] = kwargs['do_start']
        if kwargs.get('do_end', None): items[1][1] = kwargs['do_end']
        if len(items[1])>2:
            if kwargs.get('do_step', None): items[1][2] = kwargs['do_step']

        if len(items)==1: return ', WHILE (%s)' % Base._tostr((items[0]), **kwargs)
        return ', %s = %s' % (Base._tostr(items[0]), ', '.join(Base._tostr(items[1], **kwargs)))
        # end of OC addition
...
class Goto_Stmt(StmtBase): # R845
    ...
    match = staticmethod(match)
    #def tostr(self, **kwargs): return 'GO TO %s' % (self.items[0]) # OC deletion
    def tostr(self, **kwargs): return 'GO TO %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
class Computed_Goto_Stmt(StmtBase): # R846
    ...
    match = staticmethod(match)
    #def tostr(self, **kwargs): return 'GO TO (%s), %s' % self.items # OC deletion
    def tostr(self, **kwargs): return 'GO TO (%s), %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Arithmetic_If_Stmt(StmtBase): # R847
    ...
    match = staticmethod(match)
    #def tostr(self, **kwargs): return 'IF (%s) %s, %s, %s' % self.items # OC deletion
    def tostr(self, **kwargs): return 'IF (%s) %s, %s, %s' % Base._tostr(self.items, **kwargs) # OC addition
    ...
    def match(string):
        for (k,v) in [\
            ...
            try:
                ...
                obj = None
            if obj is not None: return obj
        #return 'UNIT', File_Unit_Number # OC deletion
        return 'UNIT', File_Unit_Number(string) # OC addition
    match = staticmethod(match)
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            ...
            if self.items[2] is None:
                #return 'READ(%s)' % (self.items[0]) # OC deletion
                return 'READ(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
            #return 'READ(%s) %s' % (self.items[0], self.items[2]) # OC deletion
            return 'READ(%s) %s' % Base._tostr((self.items[0], self.items[2]), **kwargs) # OC addition
        assert self.items[1] is not None, `self.items`
        if self.items[2] is None:
            #return 'READ %s' % (self.items[1]) # OC deletion
            return 'READ %s' % Base._tostr((self.items[1]), **kwargs) # OC addition
        #return 'READ %s, %s' % (self.items[1], self.items[2]) # OC deletion
        return 'READ %s, %s' % Base._tostr((self.items[1], self.items[2]), **kwargs) # OC addition
...
items : (Io_Control_Spec_List, Output_Item_List)
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if self.items[1] is None: return 'WRITE(%s)' % (self.items[0]) # OC deletion
        #return 'WRITE(%s) %s' % tuple(self.items) # OC deletion
        if self.items[1] is None: return 'WRITE(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
        return 'WRITE(%s) %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
items : (Format, Output_Item_List)
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if self.items[1] is None: return 'PRINT %s' % (self.items[0]) # OC deletion
        #return 'PRINT %s, %s' % tuple(self.items) # OC deletion
        if self.items[1] is None: return 'PRINT %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        return 'PRINT %s, %s' % tuple(Base._tostr(self.items, **kwargs)) # OC addition
...
class Io_Implied_Do_Control(Base): # R919
    ...
    def tostr(self, **kwargs):
        if self.items[3] is not None:
            #return '%s = %s, %s, %s' % (self.items) # OC deletion
            return '%s = %s, %s, %s' % Base._tostr((self.items), **kwargs) # OC addition
        #return '%s = %s, %s' % (self.items[:-1]) # OC deletion
        return '%s = %s, %s' % Base._tostr((self.items[:-1]), **kwargs) # OC addition
...
items : (File_Unit_Number, Position_Spec_List)
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            assert self.items[1] is None, `self.items`
            #return 'BACKSPACE %s' % (self.items[0]) # OC deletion
            return 'BACKSPACE %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'BACKSPACE(%s)' % (self.items[1]) # OC deletion
        return 'BACKSPACE(%s)' % Base._tostr((self.items[1]), **kwargs) # OC addition
...
items : (File_Unit_Number, Position_Spec_List)
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            assert self.items[1] is None, `self.items`
            #return 'ENDFILE %s' % (self.items[0]) # OC deletion
            return 'ENDFILE %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'ENDFILE(%s)' % (self.items[1]) # OC deletion
        return 'ENDFILE(%s)' % Base._tostr((self.items[1]), **kwargs) # OC addition
...
items : (File_Unit_Number, Position_Spec_List)
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            assert self.items[1] is None, `self.items`
            #return 'REWIND %s' % (self.items[0]) # OC deletion
            return 'REWIND %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'REWIND(%s)' % (self.items[1]) # OC deletion
        return 'REWIND(%s)' % Base._tostr((self.items[1]), **kwargs) # OC addition
...
items : (File_Unit_Number, Position_Spec_List)
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            assert self.items[1] is None, `self.items`
            #return 'FLUSH %s' % (self.items[0]) # OC deletion
            return 'FLUSH %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'FLUSH(%s)' % (self.items[1]) # OC deletion
        return 'FLUSH(%s)' % Base._tostr((self.items[1]), **kwargs) # OC addition
...
items : (Inquire_Spec_List, Scalar_Int_Variable, Output_Item_List)
    ...
    def tostr(self, **kwargs):
        if self.items[0] is None:
            ...
            #return 'INQUIRE(IOLENGTH=%s) %s' % (self.items[1:]) # OC deletion
            return 'INQUIRE(IOLENGTH=%s) %s' % Base._tostr((self.items[1:]), **kwargs) # OC addition
        #return 'INQUIRE(%s)' % (self.items[0]) # OC deletion
        return 'INQUIRE(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
    ...
    def tostr(self, **kwargs):
        ...
        if isinstance(rest, (Data_Edit_Desc, Data_Edit_Desc_C1002)):
            if rpart is not None:
                #return '%s%s' % (rpart, rest) # OC deletion
                return '%s%s' % Base._tostr((rpart, rest), **kwargs) # OC addition
            #return '%s' % (rest) # OC deletion
            return '%s' % Base._tostr((rest), **kwargs) # OC addition
        if rpart is not None:
            #return '%s(%s)' % (rpart, rest) # OC deletion
            return '%s(%s)' % Base._tostr((rpart, rest), **kwargs) # OC addition
        #return '(%s)' % (rest) # OC deletion
...
::
    ...
    def tostr(self, **kwargs):
        ...
        if c in ['F', 'D']:
            if self.items[2] is None:
                #return '%s%s' % (c, self.items[1]) # OC deletion
                return '%s%s' % Base._tostr((c, self.items[1]), **kwargs) # OC addition
            #return '%s%s.%s' % (c, self.items[1], self.items[2]) # OC deletion
            return '%s%s.%s' % Base._tostr((c, self.items[1], self.items[2]), **kwargs) # OC addition
        if c in ['E', 'EN', 'ES', 'G']:
            if self.items[3] is None:
                #return '%s%s.%s' % (c, self.items[1], self.items[2]) # OC deletion
                return '%s%s.%s' % Base._tostr((c, self.items[1], self.items[2]), **kwargs) # OC addition
            #return '%s%s.%sE%s' % (c, self.items[1], self.items[2], self.items[3]) # OC deletion
            return '%s%s.%sE%s' % Base._tostr((c, self.items[1], self.items[2], self.items[3]), **kwargs) # OC addition
        ...
        if c=='DT':
            if self.items[1] is None:
                if self.items[2] is None:
                    ...
                    #return '%s(%s)' % (c, self.items[2]) # OC deletion
                    return '%s(%s)' % Base._tostr((c, self.items[2]), **kwargs) # OC addition
            else:
                if self.items[2] is None:
                    #return '%s%s' % (c, self.items[1]) # OC deletion
                    return '%s%s' % Base._tostr((c, self.items[1]), **kwargs) # OC addition
...
items : ({R, K, None}, {'/', 'P', ':'})
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            #return '%s%s' % (self.items) # OC deletion
            return '%s%s' % Base._tostr((self.items), **kwargs) # OC addition
        #return '%s' % (self.items[1]) # OC deletion
        return '%s' % Base._tostr((self.items[1]), **kwargs) # OC addition
...
class Use_Stmt(StmtBase): # R1109
    ...
    def tostr(self, **kwargs):
        ...
        if self.items[0] is not None:
            #s += ', %s' % (self.items[0]) # OC deletion
            s += ', %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #s += ' :: %s%s' % (self.items[1], self.items[2]) # OC deletion
        s += ' :: %s%s' % Base._tostr((self.items[1], self.items[2]), **kwargs) # OC addition
        if self.items[3] is not None:
            #s += ' %s' % (self.items[3]) # OC deletion
            s += ' %s' % Base._tostr((self.items[3]), **kwargs) # OC addition
        return s
...
class Rename(Base): # R1111
    ...
    def tostr(self, **kwargs):
        if not self.items[0]:
            #return '%s => %s' % self.items[1:] # OC deletion
            return '%s => %s' % Base._tostr(self.items[1:], **kwargs) # OC addition
        #return '%s(%s) => %s(%s)' % (self.items[0], self.items[1],self.items[0], self.items[2]) # OC deletion
        return '%s(%s) => %s(%s)' % Base._tostr((self.items[0], self.items[1],self.items[0], self.items[2]), **kwargs) # OC addition
...
::
    ...
    def tostr(self, **kwargs):
        ...
        #return 'BLOCK DATA %s' % self.items # OC deletion
        return 'BLOCK DATA %s' % Base._tostr(self.items, **kwargs) # OC addition
    def get_name(self):
        return self.items[0]
...
items : ({Generic_Spec, 'ABSTRACT'},)
    ...
    def tostr(self, **kwargs):
        ...
        if self.items[0] is None:
            return 'INTERFACE'
        #return 'INTERFACE %s' % (self.items[0]) # OC deletion
        return 'INTERFACE %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
items : (Procedure_Name_List, )
    ...
    def tostr(self, **kwargs):
        #return 'MODULE PROCEDURE %s' % (self.items[0]) # OC deletion
        return 'MODULE PROCEDURE %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
items : (Proc_Interface, Proc_Attr_Spec_List, Proc_Decl_List)
    ...
    def tostr(self, **kwargs):
        ...
        if self.items[1] is not None:
            #r += ', %s ::' % (self.items[1]) # OC deletion
            r += ', %s ::' % Base._tostr((self.items[1]), **kwargs) # OC addition
        #return '%s %s' % (r, self.items[2]) # OC deletion
        return '%s %s' % Base._tostr((r, self.items[2]), **kwargs) # OC addition
...
items : (Procedure_Designator, Actual_Arg_Spec_List)
    ...
    match = staticmethod(match)
    def tostr(self, **kwargs):
        #if self.items[1] is None: return 'CALL %s' % (self.items[0]) # OC deletion
        #return 'CALL %s(%s)' % self.items # OC deletion
        if self.items[1] is None: return 'CALL %s' % Base._tostr((self.items[0]), **kwargs) # OC addition
        return 'CALL %s(%s)' % Base._tostr(self.items, **kwargs) # OC addition
...
class Alt_Return_Spec(Base): # R1222
    ...
    def match(string):
        ...
        return Label(line),
    match = staticmethod(match)
    #def tostr(self, **kwargs): return '*%s' % (self.items[0]) # OC deletion
    def tostr(self, **kwargs): return '*%s' % Base._tostr((self.items[0]), **kwargs) # OC addition
...
class Function_Stmt(StmtBase): # R1224
    ...
    def tostr(self, **kwargs):
        ...
        if prefix is not None:
            #s = '%s FUNCTION %s' % (prefix, name) # OC deletion
            s = '%s FUNCTION %s' % Base._tostr((prefix, name), **kwargs) # OC addition
        else:
            #s = 'FUNCTION %s' % (name) # OC deletion
            s = 'FUNCTION %s' % Base._tostr((name), **kwargs) # OC addition
        if dummy_args is not None:
            #s += '(%s)' % (dummy_args) # OC deletion
            s += '(%s)' % Base._tostr((dummy_args), **kwargs) # OC addition
        else:
            s += '()'
        if suffix is not None:
            #s += ' %s' % (suffix) # OC deletion
            s += ' %s' % Base._tostr((suffix), **kwargs) # OC addition
        return s
...
class Suffix(Base): # R1229
    ...
    def tostr(self, **kwargs):
        if self.items[1] is None:
            #return 'RESULT(%s)' % (self.items[0]) # OC deletion
            return 'RESULT(%s)' % Base._tostr((self.items[0]), **kwargs) # OC addition
        #return 'RESULT(%s) %s' % self.items # OC deletion
        return 'RESULT(%s) %s' % Base._tostr(self.items, **kwargs) # OC addition
...
class Subroutine_Stmt(StmtBase): # R1232
    ...
    def tostr(self, **kwargs):
        if self.items[0] is not None:
            #s = '%s SUBROUTINE %s' % (self.items[0], self.items[1]) # OC deletion
            s = '%s SUBROUTINE %s' % Base._tostr((self.items[0], self.items[1]), **kwargs) # OC addition
        else:
            #s = 'SUBROUTINE %s' % (self.items[1]) # OC deletion
            s = 'SUBROUTINE %s' % Base._tostr((self.items[1]), **kwargs) # OC addition
        if self.items[2] is not None:
            #s += '(%s)' % (self.items[2]) # OC deletion
            s += '(%s)' % Base._tostr((self.items[2]), **kwargs) # OC addition
        if self.items[3] is not None:
...
items : (Entry_Name, Dummy_Arg_List, Suffix)
    ...
    def tostr(self, **kwargs):
        ...
        if suffix is None:
            if args is None:
                #return 'ENTRY %s()' % (name) # OC deletion
                return 'ENTRY %s()' % Base._tostr((name), **kwargs) # OC addition
            #return 'ENTRY %s(%s)' % (name, args) # OC deletion
            return 'ENTRY %s(%s)' % Base._tostr((name, args), **kwargs) # OC addition
        elif args is None:
            #return 'ENTRY %s() %s' % (name, suffix) # OC deletion
            return 'ENTRY %s() %s' % Base._tostr((name, suffix), **kwargs) # OC addition
        #return 'ENTRY %s(%s) %s' % (name, args, suffix) # OC deletion
        return 'ENTRY %s(%s) %s' % Base._tostr((name, args, suffix), **kwargs) # OC addition
...
class Return_Stmt(StmtBase): # R1236
    ...
    def tostr(self, **kwargs):
        ...
        #return 'RETURN %s' % self.items # OC deletion
        return 'RETURN %s' % Base._tostr(self.items, **kwargs) # OC addition
